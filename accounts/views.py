...
from django.views.generic import View, FormView, RedirectView, DetailView, CreateView
from django.conf import settings

from ecommerce_cart.models import Order, Cart
from ecommerce_category.models import Category
from ecommerce_profile.views import profile_view
from .models import Activation
from .utils import (
    send_activation_email, send_reset_password_email, send_forgotten_username_email, send_activation_change_email,
)
from .forms import *
from ecommerce_cart.views import MyCartView
...
class ChangeProfileView(LoginRequiredMixin, FormView):
    ...
    def get_context_data(self, **kwargs):
        ...
        product_count_order = Order.objects.all()
        count_p_order = [c.cart.cartproduct_set.count() for c in product_count_order]
        context['cp'] = MyCartView.as_view
        context['fav_count'] = self.request.user.user_favourite.all()
        context['order_count'] = order_count
        context['product_order_count'] = sum(count_p_order)
    ...
    def get_initial(self):
        ...
        profile_ = self.request.user.profile
        initial = super().get_initial()
        initial['first_name'] = user.first_name
        initial['last_name'] = user.last_name
        initial['phone_number'] = profile_.phone_number
        return initial
    ...
    def form_valid(self, form):
        ...
        profile_.phone_number = form.cleaned_data['phone_number']
        profile_.save()
        user.save()

        messages.success(self.request, _('پروفایل شما با موفقیت بروزرسانی شد.'))
